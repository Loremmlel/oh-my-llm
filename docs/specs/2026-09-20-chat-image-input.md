...
- 本次不扩展设置交换、同步或收藏格式，不能把它们当作图片备份方式。未给模型配置推测视觉能力；不支持图片的服务商错误沿用聊天内联错误显示。
...
协议格式、顺序、不可变性与预算由 `llm_image_input_test.dart` 验证；文件系统与解码失败由 `file_chat_image_store_test.dart` 验证；迁移和 SQL 引用回读由 `chat_image_migration_test.dart` 及现有最低版本迁移测试验证。Chat 适配器验证 Messages 合并与缺图拒绝。页面测试覆盖选择、遮罩、预览、移除、图片／文本粘贴、编辑取消和异步过期结果。
