...
| 鼠标型号 | 待填写（真实硬件验证前未知） |
| 驱动 / 厂商软件 | 待填写（如 Logitech G HUB / 雷蛇 Synapse / 无） |
| 侧键映射 | 待填写（「后退」默认 XButton2 / 驱动重映射为 Browser Back / 其他） |
| Smoke 执行时间 | 待填写（本表最后更新：2026-08-22） |
...
> **当前执行状态：** 所有行均需用户以真实鼠标在 Windows 桌面交互执行，当前一律 `PENDING`，缺失前置条件为「待用户真实鼠标验证」。

| # | 用例 | 前置条件 | 状态 | 结果 / 缺失前置条件 |
|---|---|---|---|---|
| 1 | 普通侧键在历史 / 收藏 / 设置 / Sync 顶层页回 Chat | 真实鼠标 + 上述各顶层页 | PENDING | 待用户真实鼠标验证 |
| 2 | 收藏夹内容、收藏详情、图片、视频子路由只退一层 | 真实鼠标 + 已进入上述子路由 | PENDING | 待用户真实鼠标验证 |
| 3 | Dialog、菜单、Drawer 先关闭自身；busy Dialog（如生成中检查点）不关闭 | 真实鼠标 + 可触发的 Dialog/菜单/Drawer/busy 场景 | PENDING | 待用户真实鼠标验证 |
| 4 | History / 收藏选择态先清选择再离开 | 真实鼠标 + 已进入选择态 | PENDING | 待用户真实鼠标验证 |
| 5 | Chat 根侧键无动作、不关窗口、不退出进程 | 真实鼠标 + 停在 `/chat` 根 | PENDING | 待用户真实鼠标验证 |
| 6 | TextField 聚焦时侧键仍返回；`Alt+Left` 只移动光标不返回 | 真实鼠标 + 聊天输入框聚焦 | PENDING | 待用户真实鼠标验证 |
| 7 | 视频全屏中侧键关闭视频页；Escape 只退出全屏、留在视频页 | 真实鼠标 + 可播放视频资源 | PENDING | 待用户真实鼠标验证 |
| 8 | 快速连续点击按物理点击次数逐层返回（如 详情 → 总览 → Chat） | 真实鼠标 + 位于多层栈内 | PENDING | 待用户真实鼠标验证 |
| 9 | 单次点击不产生双退；若出现，记录驱动映射与事件来源并保持 `FAIL` | 真实鼠标 + 观察 | PENDING | 待用户真实鼠标验证 |
| 10 | browserBack 映射设备走键盘路径同样返回 | 驱动把侧键映射为 Browser Back 键的设备（若有） | PENDING | 无对应硬件/驱动映射时保持 PENDING |
