...
## 2026-09-17：长会话存储与列表加载

实施分支：`codex/fix-agent-storage-scaling`。完整存储与加载边界见[长会话存储说明](2026-09-17-storage-scaling.md)。

- 草稿独立写入、作品目录仅查询元数据；历史消息和前缀共享存储，轮次快照保留完整恢复能力。
- schema v21 保留全部已发布迁移，转换旧会话、运行和撤回快照；原文、原生推理信息和总结均保留。
- 主执行流原有虚拟列表继续使用；剧情状态页正文改为按需构建，轮次历史每次加载 20 条。列表省略历史链，检查输入时再完整读取。
- 子任务详情只查询直接子任务，避免扫描并加载整个会话的运行详情。
