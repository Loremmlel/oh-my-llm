...
- `AppDatabase`：`.open()`（生产，`getApplicationSupportDirectory`）/ `.inMemory()`（测试）/ `.forPath()`（跨 Isolate）。构造自动 `_configure()`（PRAGMA）+ `_initializeSchema()`（schema 版本校验与顺序迁移）。
- **持久顺序迁移链**：schema 用 `PRAGMA user_version` 表达，当前版本为 `AppDatabase.currentSchemaVersion`。全新数据库直接创建完整当前 schema 并标记当前版本；等于当前版本的数据库正常打开；仓库仍支持的历史版本必须按版本顺序逐步迁移到当前版本，不得跳过中间迁移或只改 `user_version`。高于当前版本、低于明确最低支持版本或 malformed schema 显式拒绝（`AppDatabaseSchemaVersionException`）。schema / `user_version` 断言用 `>=`，不用 `==`。
- **已发布迁移不滚动退役**：某个 schema 迁移进入发布版本后，其实现、合法旧版本 fixture 和数据保留测试随迁移链长期保留；新增版本只追加下一步迁移，不得因为引入新版本而删除前一步。普通 schema PR 不得提高最低支持版本；若客观上必须放弃历史版本，需用户明确授权，并用独立兼容性 PR 说明受影响数据、迁移/导出路径和回滚方案。
- 历史 JSON / settings 交换格式别名遵循同样的有界生命周期：当前格式是唯一 canonical，退出支持的旧格式必须显式失败，不做静默 fallback。
- **compatibility shim 必须有退出条件**：数据库已发布 schema 迁移不属于临时 shim；其他兼容层新增时同时说明为什么需要、位于哪个边界、何时可删除、由哪些测试保护，一次性 canonicalization 类代码不得无条件变成永久 fallback。
...
- **工具与续接边界**：共享层只提出工具调用，不执行工具。只有完整终态的 `LlmToolCall` 可交给调用方；每轮最多 32 个调用，单个参数／结果最多 1 MiB UTF-8。`LlmAssistantTurn` 的原生回放必须保持相同协议、解析后的 endpoint 和模型；显式切换目标后使用 `.portable` 仅保留公共正文与函数工具，由输入编码器生成目标请求，不构造模拟原生 envelope。两种历史都必须一次提交全部 pending 结果；Responses 原生 reasoning、Messages thinking 签名不能用显示摘要替换。缺少可靠原生输出时模型结果的 `assistantTurn` 为 null，调用方不得自行猜补，也不能用公共历史替代模型终态执行工具。
...
- **一个产品契约只有一个主 owner。** 矩阵覆盖放在能暴露根因的最低稳定 seam：纯规则归 domain / parser，持久化归 repository / migration，异步转移归 controller / run，页面只归用户可见交互，composition / integration 只归真实 wiring。
- **上层不重演下层矩阵。** 上层默认最多保留一条 canonical 成功路径，以及一条只会在本层发生的关键失败。若无法说明「哪种故障机制只有这一层的测试能捕获」，则不得新增或保留重复用例。
- 协议、平台、视口、终态等等价类在 owner 层做完整参数化；上层只选一个代表值，禁止再做笛卡尔积。只有各等价类会经过不同 adapter 或引入不同失败模式时才逐一保留。
- 安全、数据丢失、已发布迁移、协议兼容和并发终态可以防御性跨层覆盖，但每层必须明确自己独有的失败原因；不得仅因为「很重要」复制同一组输入和断言。

### 缺陷回归测试补充标准

- **「回归测试」是用例的来源，不是永久保留标签。** 所有自动测试都在防回归；缺陷修复新增用例时，必须同时明确它保护的长期产品契约、主 owner 和本层独有失败机制，不得只记录当时的 Issue 输入。
...
- 测试名称描述触发条件和预期行为，不写临时 Issue、审查或阶段编号。**当同层或更低稳定 seam 已有更小、更强的用例能捕获同一回归时，原事故用例必须合并或删除。** 清理时指出继续承担该契约的 owner；不以「它曾经抓到 bug」作为单独保留理由。
- 缺陷修复与其聚焦回归测试原则上放在同一个功能提交中；不得把无关覆盖率补测混入该提交。

### 测试清理与去重

- 删除测试前必须回答：哪个具体产品行为退化时该测试会独立失败；删除后哪个现存测试仍验证同一契约。无法指出产品行为的构造器、类型、框架转发或恒真断言应删除。
- 行覆盖相同或操作相似不代表重复；应比较**长期产品契约和预期失败机制**。测试层级不同本身不构成保留理由；只有上层真实 wiring、生命周期或交互会引入下层无法捕获的失败时，才属于互补验证。
...
- 当前 schema 契约集中验证一次；每个已发布迁移步骤保留合法旧版本 fixture、数据保留和迁移语义测试，不重复验证当前 schema 的全部列。新增 schema 版本时还要从最低支持版本执行完整顺序迁移到当前版本，证明迁移链没有断裂；历史逐级迁移测试不得随新版本滚动删除。

### 慢分片治理（强制）

- **先测量，后删减。** 至少用同一命令运行三轮并取中位数；单次最好 / 最差成绩、reporter 显示时间或主观「感觉慢」不能作为结论。性能分析可增加 `--file-reporter json:logs/<name>.json`，用 `testStart` / `testDone` 事件定位文件和用例。
- 分开「编译 / load 固定成本」与「用例执行成本」。只删便宜的纯 Dart 测试往往不会加快全量；优先处理重复 `pumpTestApp`、路由 / Dialog 动画、重建整棵 Provider 树和同一流程的跨层重演。
- 修改时先用固定文件集做单变量前后对比，再跑三轮全量。目标分片变快但全量中位数未改善时，必须如实报告「局部收益」，不得宣称全仓提速。
- 有限动画只用命名的 settle helper 等待，步进默认与 Flutter 的 100ms 一致；更小步进会额外渲染中间帧，除非测试确实断言该中间态，不得为「更稳」下调。
- 结果必须同时报告节点数、墙钟中位数、reporter 中位数、原始行覆盖率与关键 owner 覆盖率。覆盖率可以适当下降，但必须解释丢失的分支类型，并证明核心 owner 仍有保护。
