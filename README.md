...
模板正文中的变量占位符支持三种类型（未带类型标记时默认为文本）：

```text
{{主角名字}}              # 文本
{{章节数:number}}         # 数字，只接受十进制整数
{{人称:select|一|二|三}}   # 单选，选项按声明顺序展示
```

- 单选变量在聊天页以下拉框展示，保存的是选项的字符串值（不是索引），调整选项顺序后历史值仍有效；
- 正文变量 `{{正文}}` 对应主输入框：不允许声明为数字或单选，且只能出现在条件块之外；
- 手动选择模板仍是唯一触发方式：在聊天页输入框左侧选择模板后才会按条件渲染，不会自动触发。

**条件块**：使用 `{{#if 条件}}`、`{{else if 条件}}`、`{{else}}`、`{{/if}}` 按顺序渲染第一个成立的分支；所有条件都不成立且没有 `else` 时输出空字符串。
...
切换单选分支时，隐藏分支中的变量字段会保留其当前草稿值，切回时自动恢复；控制变量始终显示。

第一版限制：不支持嵌套条件块、多个条件的布尔组合、模板引用、预览、选项内容中的 `|` 转义以及字面量 `{{` 转义。

**设置导出的兼容性**：Settings 导出格式当前为 v8。较旧的受支持快照（v5-v7）导入时会自动逐级迁移到 v8；较早版本的应用会按未来格式明确拒绝 v8 导出。

**应用降级提示**：条件模板语法由本版本引入。若随后安装不包含该功能的旧版本并继续使用同一份本地数据库，旧版本虽然可以读取同一 SQLite 行，但会把控制标签和多个分支当作普通文本一起渲染，可能生成错误消息。请在降级前备份/导出设置，并避免在旧版本中编辑或发送条件模板。
...
支持目录浏览、路径导航、图片查看、视频播放与随机播放。Windows 与 Android 共用同一浏览界面：Windows 直接访问连接 Tab 中配置的本地媒体根目录（不经 HTTP）；Android 通过已配对的局域网 peer 访问同一逻辑根目录。网格按父级可用宽度自适应列数，并提供紧凑/标准/舒适三档设备本地密度（Windows 默认紧凑、Android 默认标准）。Windows 本地浏览已实现并通过自动化门禁；真实视频播放的手动验证（实际视频格式兼容、中文路径播放等）尚未执行，待用户本机补做。

视频播放器操控按平台区分：Windows 采用桌面键鼠（`←`/`→` 快退/快进 5 秒，`→` 长按 400ms 临时 3 倍速，`↑`/`↓` 与垂直滚轮调音 5%，`M` 静音，`F`/双击原生全屏，`Escape` 退出全屏或关闭，单击播放/暂停）；Android 保留触摸手势（单击控制栏、双击 ±15 秒、长按 3 倍速、横拖 Seek）。完整的分平台操控契约见 [PRD「六、视频播放器」](docs/视频局域网广播-prd.md)。
