...
├── core/
│   ├── constants/              # 响应式断点
│   ├── logging/                # AppLogger 单例（app_log.txt，网络日志记录）
...
| 模块 | 位置 | 测试数 | 覆盖范围 |
|-----|------|--------|---------|
| Favorites Widget | `test/features/favorites/` | 15 | 主页、详情页、收藏夹管理对话框 |
| Favorites Controller | `test/features/favorites/application/` | 20 | 收藏和收藏夹 CRUD、过滤、级联 |
| Chat↔Favorites Flow | `test/features/chat/chat_screen/chat_screen_favorites_cases.dart` | 6 | 书签按钮、对话框、新建收藏夹流程 |
| ChatSessionsController | `test/features/chat/application/chat_sessions_controller_test.dart` | 24 | 会话增删改、消息树编辑、重试、错误处理、内联错误显示 |
| AppDatabase Migration | `test/core/persistence/app_database_migration_test.dart` | 17 | V1-V3 schema、外键级联、索引、模型显示名称列 |
| AppShellScaffold | `test/app/shell/app_shell_scaffold_test.dart` | 14 | 响应式布局、导航栏/Rail 切换、路由 |
| ChatChunkParser | `test/features/chat/data/` | 20+ | SSE 解析、`<thought>` 标签、多种 thinking 字段、节流合并 |
| VendorPayloadAdapters | `test/features/chat/data/` | 15+ | 各厂商 API 差异、payload 构建、Google/DeepSeek/Official 特性 |
