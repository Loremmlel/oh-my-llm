...
| 剧情状态与正文 | AgentStoryState / AgentStoryRound / AgentStore | 写作、状态更新与撤回规格 | 三张只读逻辑表按字段纵向展示长文本，每表先显示 20 行，可加载更多；正文按轮次展开，撤回记录从历史入口回看 | 状态事务、撤回与页面测试 |
...
同一作品的新会话共享资料和剧情状态，拥有独立草稿、历史和配置快照。用户更新设定后，新会话首次运行采用最新版，已有会话继续使用原版。配置对话框运行中只读；关闭未保存编辑需要确认。人物卡只编辑一份完整作者设定，动态状态由状态 Agent 按正式正文维护。正文编辑、人物卡编辑和配置编辑复用相同的弹窗滚动与保存行为。窄屏将会话选择放在独立行，动作按钮允许换行。
