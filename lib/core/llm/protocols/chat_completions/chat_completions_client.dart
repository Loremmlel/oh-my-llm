...
class ChatCompletionsClient extends LlmClient {
  ...
  Stream<LlmEvent> generate(LlmRequest request, LlmCallControl control) async* {
    ...
    while (true) {
      ...
      // 每次请求独立的 parser：参数的跨 chunk 状态不跨请求复用。
      ...
      final textResult = accumulated.finish();
      final native = <String, Object?>{
        'role': 'assistant',
        'content': textResult.content,
        if (textResult.reasoningContent.isNotEmpty)
          'reasoning_content': textResult.reasoningContent,
        if (calls.isNotEmpty)
          'tool_calls': [
      ...
      final result = accumulated.finish(
        ...
        nativeItems: [native],
        toolCalls: calls,
        stopKind: calls.isEmpty ? null : LlmStopKind.toolCalls,
      );
