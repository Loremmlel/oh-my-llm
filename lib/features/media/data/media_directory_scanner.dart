...
class MediaDirectoryScanner {
  ...
  Future<List<FileItem>> scan(String relativePath) async {
    ...
    final normalizedPath = relativePath.isEmpty || relativePath == '/'
        ? '/'
    ...
    // 解析所有符号链接，同时规范化路径
    final resolvedPath = Directory(joined).resolveSymbolicLinksSync();

    // 路径穿越检测：Windows 忽略大小写比对
    if (!_isPathUnderRoot(resolvedPath)) {
