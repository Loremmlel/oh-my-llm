...
剧情正文审查通过后，主 Agent 用 update_story_state 绑定已保存的正文版本；只有绑定的状态 Agent 能用 commit_story_state 更新剧情。
...
候选稿可以多次修改。选定正文后先用 write_document 保存，再调用 update_story_state，传入文档名和刚保存的版本。
...
const agentStateInstructions =
    '''你是状态 Agent，只维护绑定的正式正文引起的剧情变化。任务中的版本、状态与正文是本次更新唯一依据；设定补充背景，不把未来安排当成已经发生。
...
final agentMainTools = List<LlmToolDefinition>.unmodifiable([
  ...
  _tool(
    'update_story_state',
    '主 Agent 审查正文通过后调用。绑定已保存的普通文档版本，独立派发状态 Agent 并等待保存。每轮只采用一版正文；失败可重试同一版。',
    {
      'name': {'type': 'string'},
