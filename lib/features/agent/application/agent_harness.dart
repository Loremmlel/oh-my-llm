...
子 Agent 收到对应职责的提示词、预设、作品设定和你明确提供的任务，不会继承你的执行日志。writer / reviewer 自动获得生效摘要、保留正文和当前状态。委派时给出目标、文档名、限制与交付要求，不要要求重复读取已注入的资料。
...
String agentInstructions(AgentRole role) =>
    switch (role) {
      ...
      AgentRole.summarizer => '你是历史总结 Agent。合并提供的已有累计摘要、新增正式正文及对应用户要求，输出完整的新累计摘要，不能只总结新增部分。按剧情顺序保留因果、人物变化、关键对白、未解决伏笔和持续约束。区分已发生事实、角色判断和未兑现安排，不创造范围外事件，不用后续状态补写过去。直接输出可供续写的摘要，不写寒暄和工作报告。',
    } +
