...
/// coordinator 投递给 controller 的事件流。
///
/// chunk 只带 content/reasoning/finish reason，不暴露 [StreamSubscription] 等
/// stream 实现细节。所有事件携带 [generationId]，controller 据此区分新旧 generation。
sealed class ChatGenerationEvent {
  const ChatGenerationEvent({required this.generationId});

  final int generationId;
}

/// generation 已启动（preparing -> streaming 转换）。
...
/// 已调度下一次 retry，进入等待窗口。
class ChatGenerationRetryScheduled extends ChatGenerationEvent {
  const ChatGenerationRetryScheduled({
    required super.generationId,
...
/// 用户 stop 已使 token 失效，正在形成中断快照。
class ChatGenerationStopped extends ChatGenerationEvent {
...
/// generation 被取消（user stop / supersede / dispose）。
class ChatGenerationCancelledEvent extends ChatGenerationEvent {
  const ChatGenerationCancelledEvent({
    required super.generationId,
    required this.reason,
  });
...
/// 关键持久化失败。
class ChatGenerationPersistenceFailedEvent extends ChatGenerationEvent {
  const ChatGenerationPersistenceFailedEvent({
    required super.generationId,
    required this.error,
  });
