...
import 'package:oh_my_llm/features/chat/data/chat_sql_codec.dart';
...
void main() {
  group('executeSaveConversations UPSERT & cleanup', () {
    ...
    test('UPSERT idempotency: same ID message updated not duplicated', () {
      ...
      executeSaveConversations(db, [conv]);
      ...
      executeSaveConversations(db, [modifiedConv]);
    ...
    test('Ghost row cleanup: removed message gone after re-save', () {
      ...
      executeSaveConversations(db, [slimConv]);
    ...
    test('Transaction rollback leaves no partial rows on failed write', () {
      ...
      executeSaveConversations(db, [baselineConv]);
      ...
      appDb.close();
      expect(
        () => executeSaveConversations(db, [failingConv]),
        throwsA(isA<Object>()),
      );
    ...
    test('Multi-conversation write: both persisted correctly', () {
      ...
      executeSaveConversations(db, [convA, convB]);
