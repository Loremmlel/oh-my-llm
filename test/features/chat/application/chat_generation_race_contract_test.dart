...
void main() {
  ...
  group(
    'serialized generation run contract',
    () {
      ...
      test(
        ...
        () async {
          ...
          int? preparingId;
          int? streamingId;
          final sub = container.listen<ChatSessionsState>(
            chatSessionsProvider,
            (previous, next) {
              final phase = next.generation?.phase;
              if (phase == ChatGenerationPhase.preparing &&
                  preparingId == null) {
    ...
    skip: '待 Task 3 串行 run 切换后启用（修复前为红，切换后转绿）',
  );
}
