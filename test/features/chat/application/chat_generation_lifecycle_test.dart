...
void main() {
  ...
  // ── ChatGenerationEvent 类型区分 ─────────────────────────

  group('ChatGenerationEvent', () {
    test('所有事件携带 generationId 以区分新旧 generation', () {
      const events = <ChatGenerationEvent>[
        ...
        ChatGenerationStopped(generationId: 7, partialContent: 'p'),
        ChatGenerationCancelledEvent(
          generationId: 7,
          reason: ChatCancelReason.userStop,
        ),
        ChatGenerationPersistenceFailedEvent(
          generationId: 7,
          error: 'save failed',
        ),
      ];
      ...
      for (final event in events) {
        expect(event.generationId, 7);
      }
      expect(events, hasLength(8));
    });

    test('chunk 携带 delta 与可选 finishReason', () {
      const chunk = ChatGenerationChunk(
        generationId: 1,
        contentDelta: 'hello',
      ...
      expect(chunk.reasoningDelta, 'think');
      expect(chunk.finishReason, 'stop');
    });

    test('retryScheduled 携带下一次 attempt 与延迟', () {
      const event = ChatGenerationRetryScheduled(
        generationId: 1,
        nextAttempt: 3,
        delay: Duration(seconds: 5),
      );

      expect(event.nextAttempt, 3);
      expect(event.delay, const Duration(seconds: 5));
    });

    test('cancelledEvent 区分取消原因', () {
      const userStop = ChatGenerationCancelledEvent(
        generationId: 1,
        reason: ChatCancelReason.userStop,
      );
      const superseded = ChatGenerationCancelledEvent(
        generationId: 1,
        reason: ChatCancelReason.superseded,
      );

      expect(userStop.reason, ChatCancelReason.userStop);
      expect(superseded.reason, ChatCancelReason.superseded);
    });
  });
}
