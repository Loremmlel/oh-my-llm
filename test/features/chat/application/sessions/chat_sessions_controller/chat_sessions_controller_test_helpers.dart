...
class ControllerTestHarness {
  ...
  /// 用真实 [ChatCompletionsClient] 构造一条会触发 SSE idle 超时的流，
  /// 保留 async* 生成器对 error/done 事件的真实调度时序。
  ///
  /// mock HTTP 返回的 SSE 流先发一条 data 行（部分内容）后永不结束，配合短
  /// [idleTimeout] 触发 [LlmHttpStreamTransport] 的 idle 超时：在同一同步栈内对
...
///
/// 配合 [ChatCompletionsClient] 的短 `streamIdleTimeout`，模拟服务器
/// 发送部分内容后长时间无响应的场景，迫使传输层触发 idle 超时
/// （在同一同步栈内连续 addError + close）。
