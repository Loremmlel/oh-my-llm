...
void main() {
  ...
  test(
    'buildTemplatedUserMessage injects body and variables into template',
    () {
  ...
  test(
    'buildTemplatedUserMessage inserts body above template without 正文 placeholder',
    () {
      final result = buildTemplatedUserMessage(
        body: '原文',
        templatePrompt: TemplatePrompt(
  ...
  test('buildTemplatedUserMessage keeps template-only content when 正文为空', () {
    final result = buildTemplatedUserMessage(
      body: '   ',
      templatePrompt: TemplatePrompt(
        id: 'tp-3',
    ...
    expect(result.content, '请输出专业版总结。');
    expect(result.userMessageSegments, const [
      UserMessageSegment(
        text: '请输出专业版总结。',
        kind: UserMessageSegmentKind.template,
