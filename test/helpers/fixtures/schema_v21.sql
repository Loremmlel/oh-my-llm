...
-- 已发布 v17 完整 schema 快照；旧版基础表沿用 v16。
...
-- 已发布 v19 schema；空 fixture 不涉及资料数据转换。
...
DROP TABLE agent_document_revisions;
...
-- 已发布 v21 schema：历史链共享前缀，草稿独立持久化。
...
CREATE TABLE agent_history_entries (
  workspace_id TEXT NOT NULL, id TEXT NOT NULL, parent_id TEXT NOT NULL, item_id TEXT NOT NULL,
  PRIMARY KEY(workspace_id, id),
  FOREIGN KEY(workspace_id) REFERENCES agent_workspaces(id) ON DELETE CASCADE,
...
CREATE INDEX idx_agent_runs_parent ON agent_runs(workspace_id, json_extract(record_json, '$.parentId'), started_at, id);
