...
class ScrollablePositionedList extends StatefulWidget {
  ...
  const ScrollablePositionedList.builder({
    ...
    this.minCacheExtent,
        ...
        assert(cacheExtent == null || minCacheExtent == null),
  ...
  /// 底层滚动列表使用的固定像素缓存范围。
  ///
  /// 非空时不再根据 viewport 尺寸计算缓存范围，也不通过 [LayoutBuilder]
  /// 监听尺寸变化。适合 viewport 会持续动画变高或变矮、子项布局开销较大的列表。
  /// 不能与 [minCacheExtent] 同时设置。
...
class _ScrollablePositionedListState extends State<ScrollablePositionedList>
  ...
  Widget build(BuildContext context) {
    final fixedCacheExtent = widget.cacheExtent;
    if (fixedCacheExtent != null) {
      return _buildLists(fixedCacheExtent);
    }

    return LayoutBuilder(
      builder: (context, constraints) {
        final cacheExtent = _cacheExtent(constraints);
        return _buildLists(cacheExtent);
      },
    );
  }

  Widget _buildLists(double cacheExtent) {
    return Listener(
